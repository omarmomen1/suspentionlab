...
function FullCarDashboard() {
  ...
  const psdRoll = useMemo(() => {
    ...
    const ddphi = results.ddphi as number[];
    const t = results.time as number[];
    if (!ddphi || ddphi.length < 64) return null;
    const n = ddphi.length, dt = t[1] - t[0], fs = 1 / dt, N = n, real = new Float64Array(N);
    for (let i = 0; i < N; i++) real[i] = ddphi[i] * 0.5 * (1 - Math.cos(2 * Math.PI * i / (N - 1)));
        ...
        <div className="w-60 shrink-0 bg-[#111113] border border-[#1e1e1e] rounded-xl overflow-y-auto custom-scrollbar">
          ...
          <div className="p-4 space-y-5">
            ...
            <ParamSection title="Inertia">
              ...
              <ParameterInput label="Roll Inertia Ix" val={params.I_x} unit="kgÂ·mÂ²" min={1} onChange={(v) => setParams({ ...params, I_x: v })} />
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0 overflow-hidden">
          {!results && !isRunning && (
            <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center"><p className="text-sm font-semibold text-gray-400 mb-1">Configure parameters and click <span className="text-ansys-yellow">Run Solver</span></p><p className="text-xs text-gray-700">7-DOF full car Â· LTI State-Space Â· Pitch & Roll</p></div>
            </div>
          )}
          ...
          {isRunning && (
            <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center"><p className="text-sm font-medium text-gray-400 mb-1">Running LTI Solverâ€¦</p></div>
            </div>
          )}
          ...
          {hasResults && r && (
            <div id="sim-report" className="flex flex-col gap-3 flex-1 overflow-hidden">
              ...
              <div className="grid grid-cols-8 gap-2 shrink-0">
                <KPICard label="Heave Æ’â‚™" value={r.f_n_heave} unit="Hz" status={r.f_n_heave >= 1.0 && r.f_n_heave <= 1.5 ? "good" : "warn"} />
                <KPICard label="Roll Æ’â‚™" value={r.f_n_roll} unit="Hz" status={r.f_n_roll >= 1.5 && r.f_n_roll <= 2.5 ? "good" : "warn"} />
                <KPICard label="Pitch Æ’â‚™" value={r.f_n_pitch} unit="Hz" status={r.f_n_pitch >= 1.2 && r.f_n_pitch <= 1.8 ? "good" : "warn"} />
                ...
                <KPICard label="RMS Roll" value={r.rms_roll_accel} unit="rad/sÂ²" status="neutral" />
                ...
                {activeTab === 0 && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 auto-rows-[350px] gap-px bg-[#181818] overflow-y-auto custom-scrollbar h-full">
                    ...
                    <PlotBox>
                      <Plot
                        data={[ 
                          { x: r.time, y: r.theta.map((v: number) => v * 180 / Math.PI), type: "scatter", mode: "lines", name: "Pitch Angle Î¸", line: { color: "#00ff87", width: 2 } },
                          { x: r.time, y: r.phi.map((v: number) => v * 180 / Math.PI), type: "scatter", mode: "lines", name: "Roll Angle Ï†", line: { color: "#bf5af2", width: 2 } } 
                        ]}
                        layout={darkLayout({ title: { text: "Attitude Angles", font: { color: "#888", size: 11 } }, xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } }, yaxis: { ...DARK_BASE.yaxis, title: { text: "Angle (Â°)", font: { color: "#444", size: 10 } } } })}
                        useResizeHandler style={{ width: "100%", height: "100%" }} config={{ responsive: true, displayModeBar: true }}
                      />
                    </PlotBox>
                    <PlotBox>
                      <Plot
                        data={[ { x: r.time, y: r.ddz_s, type: "scatter", mode: "lines", name: "Heave Accel zÌˆ_s", line: { color: "#f2a900", width: 2 }, fill: "tozeroy", fillcolor: "rgba(242,169,0,0.05)" } ]}
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({ title: { text: "Angular Accelerations", font: { color: "#888", size: 11 } }, xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } }, yaxis: { ...DARK_BASE.yaxis, title: { text: "Accel (rad/sÂ²)", font: { color: "#444", size: 10 } } } })}
                        useResizeHandler style={{ width: "100%", height: "100%" }} config={{ responsive: true, displayModeBar: true }}
                      />
                    </PlotBox>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 auto-rows-[350px] gap-px bg-[#181818] overflow-y-auto custom-scrollbar h-full">
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({ title: { text: "Roll vs Pitch Space", font: { color: "#888", size: 11 } }, xaxis: { ...DARK_BASE.xaxis, title: { text: "Roll Ï† (Â°)", font: { color: "#444", size: 10 } } }, yaxis: { ...DARK_BASE.yaxis, title: { text: "Pitch Î¸ (Â°)", font: { color: "#444", size: 10 } } } })}
                        useResizeHandler style={{ width: "100%", height: "100%" }} config={{ responsive: true, displayModeBar: true }}
                      />
                    </PlotBox>
                    <PlotBox>
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({ title: { text: "Roll Phase Portrait", font: { color: "#888", size: 11 } }, xaxis: { ...DARK_BASE.xaxis, title: { text: "Ï† (Â°)", font: { color: "#444", size: 10 } } }, yaxis: { ...DARK_BASE.yaxis, title: { text: "Ï†Ì‡ (Â°/s)", font: { color: "#444", size: 10 } } } })}
                        useResizeHandler style={{ width: "100%", height: "100%" }} config={{ responsive: true, displayModeBar: true }}
                      />
                    </PlotBox>
                    <PlotBox>
                  ...
                  <div className="grid grid-cols-1 auto-rows-[400px] gap-px bg-[#181818] overflow-y-auto custom-scrollbar h-full">
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({ title: { text: "Lateral Load Transfer (Î”Fz)", font: { color: "#888", size: 11 } }, xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } }, yaxis: { ...DARK_BASE.yaxis, title: { text: "Load Transfer (N)", font: { color: "#444", size: 10 } } } })}
                  ...
                  <div className="grid grid-cols-1 grid-rows-1 h-full gap-px bg-[#181818]">
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({ title: { text: "PSD â€” Roll Acceleration", font: { color: "#888", size: 11 } }, xaxis: { ...DARK_BASE.xaxis, title: { text: "Frequency (Hz)", font: { color: "#444", size: 10 } }, type: "log", range: [Math.log10(0.1), Math.log10(50)] }, yaxis: { ...DARK_BASE.yaxis, title: { text: "PSD ((rad/sÂ²)Â²/Hz)", font: { color: "#444", size: 10 } }, type: "log" } })}
                        useResizeHandler style={{ width: "100%", height: "100%" }} config={{ responsive: true, displayModeBar: false }}
                      />
                    </PlotBox>
                  </div>
