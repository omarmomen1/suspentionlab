...
function QuarterCarDashboard() {
  ...
  // â”€â”€â”€ Derived: PSD from acceleration time series (Welch-style via FFT) â”€â”€â”€â”€â”€
  const psdData = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) real[i] = ddz[i];
    // Hann window
    for (let i = 0; i < N; i++) real[i] *= 0.5 * (1 - Math.cos(2 * Math.PI * i / (N - 1)));
    // FFT via DFT (simple, N < 5000)
    const freqs: number[] = [];
    const psd:   number[] = [];
    ...
    for (let k = 0; k <= half; k++) {
      ...
      for (let n2 = 0; n2 < N; n2++) {
        ...
        re += real[n2] * Math.cos(angle);
        im -= real[n2] * Math.sin(angle);
      }
      const mag2 = (re * re + im * im) / (N * fs);
      freqs.push(k * fs / N);
      psd.push(k > 0 && k < half ? 2 * mag2 : mag2);
    }
    return { freqs, psd };
  }, [results]);
      ...
      <div className="flex items-center justify-between shrink-0 pb-3 border-b border-[#1e1e1e]">
        <div className="flex flex-col">
          <span className="text-[9px] font-bold tracking-[0.25em] text-ansys-yellow uppercase">
            2-DOF Vertical Dynamics Â· RK45 ODE
          </span>
        ...
        <div className="w-60 shrink-0 bg-[#111113] border border-[#1e1e1e] rounded-xl overflow-y-auto custom-scrollbar">
          ...
          <div className="p-4 space-y-5">
            ...
            <ParamSection title="Suspension">
              ...
              <ParameterInput label="Damping"        val={params.c}   unit="NÂ·s/m" step={50}   onChange={(v) => setParams({ ...params, c:   v })} />
            ...
            <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-700">
                  2-DOF quarter car Â· RK45 ODE Â· Impedance matrix FRF Â· ISO 2631-1 Wk weighting
                </p>
          ...
          {isRunning && (
            <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-700">Computing eigenvalues Â· FRF Â· time response Â· metrics</p>
              </div>
            </div>
          )}
            ...
            <div id="sim-report" className="flex flex-col gap-3 flex-1 overflow-hidden">
              ...
              <div className="grid grid-cols-8 gap-2 shrink-0">
                <KPICard
                  ...
                  value={r.f_n_s}
                  unit="Hz"
                  sub={`Ï‰â‚™ = ${r.omega_n_s.toFixed(2)} rad/s`}
                ...
                />
                <KPICard
                  label="Damping Ratio Î¶"
                  value={r.zeta_s}
                  sub={`c_crit = ${r.c_crit_s.toFixed(0)} NÂ·s/m`}
                ...
                {activeTab === 1 && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 auto-rows-[350px] gap-px bg-[#181818] overflow-y-auto custom-scrollbar h-full">
                    ...
                    <PlotBox>
                      <Plot
                        data={[
                          { x: r.freq_hz, y: r.bode_phase_deg, type: "scatter", mode: "lines", name: "Phase (Â°)",
                            line: { color: "#ff9f0a", width: 2 } },
                        ]}
                        layout={darkLayout({
                          title: { text: "Bode â€” Phase", font: { color: "#888", size: 11 } },
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Frequency (Hz)", font: { color: "#444", size: 10 } }, type: "log" },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "Phase (Â°)", font: { color: "#444", size: 10 } }, range: [-200, 10] },
                          shapes: [
                          ...
                          annotations: [
                            { x: 0.02, xref: "paper", y: -90, yref: "y", text: "âˆ’90Â°", showarrow: false, font: { color: "#555", size: 9 }, xanchor: "left" },
                            { x: 0.02, xref: "paper", y: -180, yref: "y", text: "âˆ’180Â°", showarrow: false, font: { color: "#555", size: 9 }, xanchor: "left" },
                          ],
                        })}
                        useResizeHandler style={{ width: "100%", height: "100%" }}
                        config={{ responsive: true, displayModeBar: true }}
                    ...
                    <PlotBox>
                      <Plot
                        data={[
                          { x: r.time, y: r.tire_load_var, type: "scatter", mode: "lines", name: "Dynamic tire load Î”F_z",
                        ...
                        layout={darkLayout({
                          ...
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "Î”F_z (N)", font: { color: "#444", size: 10 } } },
                          shapes: [
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          ...
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "Body Accel (m/sÂ²)", font: { color: "#444", size: 10 } } },
                        })}
                        useResizeHandler style={{ width: "100%", height: "100%" }}
                        config={{ responsive: true, displayModeBar: true }}
                      />
                      ...
                      <Plot
                        data={[
                          { x: psdData.freqs, y: psdData.psd, type: "scatter", mode: "lines",
                            name: "PSD áº‘Ìˆ_s", fill: "tozeroy", fillcolor: "rgba(255,45,85,0.06)",
                        ...
                        layout={darkLayout({
                          ...
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "PSD ((m/sÂ²)Â²/Hz)", font: { color: "#444", size: 10 } }, type: "log" },
                          shapes: [
