// TypeScript type safety fully enabled — @ts-nocheck removed (was CRIT-05)
...
const ROAD_PROFILES = [
  ...
  { label: "Multi-Harmonic Random",   value: "random"  },
  { label: "Impulse",                 value: "impulse" },
  { label: "ISO 8608 Stochastic",     value: "iso8608" },
];

const ISO_CLASSES = ["A", "B", "C", "D", "E", "F", "G", "H"] as const;
type IsoClass = typeof ISO_CLASSES[number];
...
const TABS = [
  ...
  { label: "Sensitivity",       icon: FlaskConical },
];
...
// MED-01 FIX: getComfortStatus must use Wk-weighted RMS (ISO 2631-1 boundary values are for Wk)
function getComfortStatus(rmsWk: number): Status {
  if (rmsWk < 0.315) return "good";
  if (rmsWk < 1.0)   return "warn";
  return "bad";
}
...
function QuarterCarDashboard() {
  ...
  // Snapshot of params used for the LAST successful simulation (MED-07)
  const [simParams, setSimParams] = useState<typeof params | null>(null);
  ...
  const [params, setParams] = useState({
    ...
    frequency:    2.0,
    duration:     5.0,
    iso_class:    "B" as IsoClass,   // ARCH-05: ISO 8608 road roughness class (A=smooth – H=very rough)
    speed_mps:    20.0,              // ARCH-05: vehicle speed for ISO 8608 temporal PSD scaling
  ...
  // MIN-07: Dismiss toast on Escape key
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => { if (e.key === "Escape") setToast(null); };
  ...
  const buildPayload = () => ({
    params: {
      m_s: params.m_s, m_u: params.m_u, k_s: params.k_s, c: params.c,
      k_t: params.k_t, MR: params.MR, c_t: params.c_t,
      damper_curve_v: params.damper_curve_v, damper_curve_f: params.damper_curve_f,
    },
    profile: {
      profile_type: params.road_profile,
      amplitude: params.amplitude,
      frequency: params.frequency,
      duration: params.duration,
      iso_class: params.iso_class,
      speed_mps: params.speed_mps,   // ARCH-05: pass ISO 8608 speed to backend
    },
  });
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setResults({ error: true, message: data.message ?? data.detail ?? "Simulation failed" });
      } else {
        setResults(data);
        setSimParams({ ...params }); // MED-07: snapshot params at the moment simulation completes
        saveSimulationToHistory("sl_sim_history_qcar", params, data);
      }
    } catch (err) {
  ...
  // ————————————————— Derived: PSD from acceleration time series (Welch-style via FFT) —————
  ...
  // MED-07: Forces computed from SNAPSHOTTED params (simParams), not current live params
  const forces = useMemo(() => {
    if (!results || results.error) return null;
    const p = simParams ?? params; // fall back to current if snapshot not yet available
    ...
    if (!zs || !zu || !vs || !vu) return null;
    const kw = p.k_s * p.MR * p.MR;
    const cw = p.c   * p.MR * p.MR;
  ...
  }, [results, simParams]); // NOT dependent on live `params`
      ...
      <div className="flex items-center justify-between shrink-0 pb-3 border-b border-[#1e1e1e]">
        <div className="flex flex-col">
          <span className="text-[9px] font-bold tracking-[0.25em] text-ansys-yellow uppercase">
            2-DOF Vertical Dynamics · BDF Stiff ODE
          </span>
          <h1 className="text-2xl font-semibold tracking-tight text-white leading-tight">Quarter Car</h1>
        </div>

        <div className="flex items-center gap-2">
          <SimHistory storageKey="sl_sim_history_qcar" onRestore={(p) => { setParams(prev => ({ ...prev, ...p })); showToast("Restored from history", "success"); }} />
          ...
          <PDFExport captureId="sim-report" fileName={`QCar_${Math.round(params.k_s)}_${Math.round(params.c)}`} />
          ...
          <button id="btn-run" onClick={runSimulation} disabled={isRunning}
            ...
            {isRunning ? (
              <><div className="w-3 h-3 border border-gray-600 border-t-gray-300 rounded-full animate-spin" />Solving…</>
            ) : (
        ...
        <div className="w-60 shrink-0 bg-[#111113] border border-[#1e1e1e] rounded-xl overflow-y-auto custom-scrollbar">
          ...
          <div className="p-4 space-y-5">
            ...
            <ParamSection title="Suspension">
              ...
              <ParameterInput label="Motion Ratio"   val={params.MR}  unit="—"     step={0.01} min={0.1} onChange={(v) => setParams({ ...params, MR:  v })} />
              <DamperLUTInput 
            ...
            <ParamSection title="Road Profile">
              ...
              {/* ARCH-05: ISO 8608 road class and speed controls */}
              {params.road_profile === "iso8608" && (
                <>
                  <div className="flex justify-between items-center gap-2">
                    <label className="text-[11px] text-gray-500 flex-1">Road Class</label>
                    <select
                      value={params.iso_class}
                      onChange={(e) => setParams({ ...params, iso_class: e.target.value as IsoClass })}
                      className="bg-[#0d0d0d] border border-[#252525] text-[11px] rounded-lg px-2 py-1.5 text-white w-[7.5rem] focus:outline-none focus:border-ansys-yellow/60">
                      {ISO_CLASSES.map((cls) => (
                        <option key={cls} value={cls}>Class {cls}</option>
                      ))}
                    </select>
                  </div>
                  <ParameterInput label="Speed" val={params.speed_mps} unit="m/s" step={5} min={1} onChange={(v) => setParams({ ...params, speed_mps: v })} />
                </>
              )}
            </ParamSection>
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0 overflow-hidden">
            ...
            <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-700">
                  2-DOF quarter car · BDF implicit stiff solver · Impedance matrix FRF · ISO 2631-1 Wk weighting
                </p>
          ...
          {isRunning && (
            <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                <p className="text-sm font-medium text-gray-400 mb-1">Running BDF stiff ODE solver…</p>
            ...
            <div id="sim-report" className="flex flex-col gap-3 flex-1 overflow-hidden">
              ...
              <div className="grid grid-cols-8 gap-2 shrink-0">
                <KPICard
                  ...
                  value={r.f_n_s ?? 0}
                  unit="Hz"
                  sub={`ωₙ = ${r.omega_n_s?.toFixed(2) ?? 'N/A'} rad/s`}
                ...
                <KPICard
                  ...
                  value={r.zeta_s ?? 0}
                  sub={`c_crit = ${r.c_crit_s?.toFixed(0) ?? 'N/A'} N·s/m`}
                  status={getDampingStatus(r.zeta_s ?? 0)}
                />
                <KPICard
                  label="Peak TX"
                  value={r.peak_transmissibility ?? 0}
                  sub={`@ ${r.freq_at_peak_tx?.toFixed(2) ?? 'N/A'} Hz`}
                  status={getTXStatus(r.peak_transmissibility ?? 0)}
                />
                <KPICard
                  label="Wk RMS Accel"
                  value={r.rms_body_accel_wk ?? r.rms_body_accel}
                  unit="m/s²"
                  sub={r.comfort_rating || "ISO 2631-1 Wk"}
                  status={getComfortStatus(r.rms_body_accel_wk ?? r.rms_body_accel)}
                />
                <KPICard
                  label="RMS Susp Travel"
                  value={(r.rms_susp_travel ?? 0) * 1000}
                  unit="mm"
                  sub={`Peak: ${((r.peak_susp_travel ?? 0) * 1000).toFixed(1)} mm`}
                  status={(r.peak_susp_travel ?? 0) < 0.08 ? "good" : (r.peak_susp_travel ?? 0) < 0.15 ? "warn" : "bad"}
                />
                <KPICard
                  label="RMS Tire Load"
              ...
              <div className="flex-1 bg-[#111113] border border-[#1e1e1e] rounded-xl overflow-hidden min-h-0">
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          ...
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "Acceleration (m/sÂ²)", font: { color: "#444", size: 10 } } },
                          shapes: [
                          ...
                          annotations: [{ x: 0.02, xref: "paper", y: 0.315, yref: "y",
                            text: "ISO 0.315 m/sÂ²", showarrow: false, font: { color: "#22c55e80", size: 9 }, xanchor: "left" }],
                        })}
                        useResizeHandler style={{ width: "100%", height: "100%" }}
                        config={{ responsive: true, displayModeBar: true }}
                      />
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 auto-rows-[350px] gap-px bg-[#181818] overflow-y-auto custom-scrollbar h-full">
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          title: { text: "Bode — Phase", font: { color: "#888", size: 11 } },
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Frequency (Hz)", font: { color: "#444", size: 10 } }, type: "log" },
                          // MIN-03: Auto-range phase axis based on actual data — clamp to [-200, 10] only as sensible default
                          ...(r.bode_phase_deg && r.bode_phase_deg.length > 0
                            ? { yaxis: { ...DARK_BASE.yaxis,
                                title: { text: "Phase (°)", font: { color: "#444", size: 10 } },
                                range: [
                                  Math.min(-10, Math.floor(Math.min(...r.bode_phase_deg) / 10) * 10 - 10),
                                  Math.max(10, Math.ceil(Math.max(...r.bode_phase_deg) / 10) * 10 + 10),
                                ] } }
                            : {}),
                          shapes: [
                    ...
                    {/* Dynamic Index — tire load variation */}
                    <PlotBox>
                      <Plot
                        data={[
                          { x: r.time, y: r.tire_load_var, type: "scatter", mode: "lines", name: "Dynamic tire load ΔF_z",
                        ...
                        layout={darkLayout({
                          ...
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "ΔF_z (N)", font: { color: "#444", size: 10 } } },
                          shapes: [
                ...
                {activeTab === 2 && forces && (
                  <div className="grid grid-cols-2 grid-rows-2 h-full gap-px bg-[#181818]">
                      ...
                      <Plot
                        data={[
                          { x: forces.relVel.map(v => v * 1000), y: forces.F_damper, type: "scatter", mode: "markers",
                            name: "Damper F–V", marker: { color: forces.F_damper.map((_, i) => i / forces.F_damper.length),
                              colorscale: [[0, "#0a1628"], [0.5, "#00aeff"], [1, "#f2a900"]] as [number, string][],
                              size: 2, opacity: 0.8 } },
                        ]}
                        layout={darkLayout({
                          title: { text: "Damper Force–Velocity Diagram", font: { color: "#888", size: 11 } },
                    ...
                    {/* Spring F–Displacement */}
                    <PlotBox>
                      <Plot
                        data={[
                          { x: r.susp_travel.map(v => v * 1000), y: forces.F_spring, type: "scatter", mode: "markers",
                            name: "Spring F–x", marker: { color: "#f2a900", size: 2, opacity: 0.7 } },
                        ]}
                        layout={darkLayout({
                          title: { text: "Spring Force–Displacement (Hysteresis)", font: { color: "#888", size: 11 } },
                  ...
                  <div className="grid grid-cols-1 auto-rows-[400px] gap-px bg-[#181818] overflow-y-auto custom-scrollbar h-full">
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          title: { text: "Phase Portrait — Sprung Mass", font: { color: "#888", size: 11 } },
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "z_s (mm)", font: { color: "#444", size: 10 } } },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "ż_s (mm/s)", font: { color: "#444", size: 10 } } },
                        })}
                        useResizeHandler style={{ width: "100%", height: "100%" }}
                        config={{ responsive: true, displayModeBar: true }}
                      />
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          title: { text: "Phase Portrait — Unsprung Mass", font: { color: "#888", size: 11 } },
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "z_u (mm)", font: { color: "#444", size: 10 } } },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "ż_u (mm/s)", font: { color: "#444", size: 10 } } },
                        })}
                        useResizeHandler style={{ width: "100%", height: "100%" }}
                        config={{ responsive: true, displayModeBar: true }}
                      />
                    ...
                    <PlotBox>
                      <div className="h-full p-5 flex flex-col justify-center gap-4">
                        ...
                        <div className="space-y-3 font-mono text-xs">
                          {[
                            { label: "Sprung ωₙ",   val: `${r.omega_n_s.toFixed(4)} rad/s`, color: "#f2a900" },
                            { label: "Sprung ζ",    val: r.zeta_s.toFixed(4),              color: getDampingStatus(r.zeta_s) === "good" ? "#34d399" : "#f59e0b" },
                            { label: "Sprung ωd",   val: `${(r.omega_n_s * Math.sqrt(Math.max(0, 1 - r.zeta_s**2))).toFixed(4)} rad/s`, color: "#aaa" },
                            { label: "Unsprung ωₙ", val: `${(r.f_n_u * 2 * Math.PI).toFixed(4)} rad/s`, color: "#00aeff" },
                            { label: "Unsprung ζ",  val: (r.zeta_u as number).toFixed(4),  color: "#aaa" },
                        ...
                        <p className="text-[9px] text-gray-700 mt-2">
                          Eigenvalues extracted from 4×4 state-space matrix A via scipy.linalg.eigvals
                        </p>
                      </div>
                    </PlotBox>
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          title: { text: "Comfort–Travel Trade-off Space", font: { color: "#888", size: 11 } },
                ...
                {activeTab === 4 && psdData && (
                  <div className="grid grid-cols-2 grid-rows-2 h-full gap-px bg-[#181818]">
                    ...
                    <PlotBox>
                      <Plot
                        data={[
                          { x: psdData.freqs, y: psdData.psd, type: "scatter", mode: "lines",
                            name: "PSD ӓ_s", fill: "tozeroy", fillcolor: "rgba(255,45,85,0.06)",
                            line: { color: "#ff2d55", width: 2 } },
                        ]}
                        layout={darkLayout({
                          title: { text: "PSD — Body Acceleration", font: { color: "#888", size: 11 } },
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Frequency (Hz)", font: { color: "#444", size: 10 } }, type: "log", range: [Math.log10(0.1), Math.log10(50)] },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "PSD ((m/s²)²/Hz)", font: { color: "#444", size: 10 } }, type: "log" },
                          shapes: [
                    ...
                    <PlotBox>
                      <Plot
                        ...
                        layout={darkLayout({
                          ...
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Frequency (Hz)", font: { color: "#444", size: 10 } }, type: "log" },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "|H(jω)| (–)", font: { color: "#444", size: 10 } } },
                          shapes: [
                      ...
                      <div className="h-full p-5 flex flex-col justify-center gap-3">
                        ...
                        <div className="mt-2 pt-3 border-t border-[#1a1a1a] font-mono text-xs">
                          <div className="flex justify-between text-gray-600">
                            ...
                            <span className="text-white">{r.rms_body_accel.toFixed(4)} m/sÂ²</span>
                          </div>
                          <div className="flex justify-between text-gray-600 mt-1">
                            <span>Wk-weighted RMS</span>
                            <span className="text-ansys-yellow">{(r.rms_body_accel_wk ?? 0).toFixed(4)} m/sÂ²</span>
                          </div>
                        </div>
                      </div>
                    </PlotBox>
                    ...
                    <PlotBox>
                      <Plot
                        data={[{
                          x: r.time,
                          y: (() => {
                            // MIN-01: O(n) running sum instead of O(n²) nested reduce
                            const acc: number[] = [];
                            let sumSq = 0;
                            (r.ddz_s as number[]).forEach((v: number, i: number) => {
                              sumSq += v * v;
                              acc.push(Math.sqrt(sumSq / (i + 1)));
                            });
                            return acc;
                          })(),
                        ...
                        layout={darkLayout({
                          ...
                          xaxis: { ...DARK_BASE.xaxis, title: { text: "Time (s)", font: { color: "#444", size: 10 } } },
                          yaxis: { ...DARK_BASE.yaxis, title: { text: "Cum. RMS (m/sÂ²)", font: { color: "#444", size: 10 } } },
                          shapes: [
                            { type: "line", x0: 0, x1: 1, xref: "paper", y0: 0.315, y1: 0.315, line: { color: "rgba(52,211,153,0.3)", dash: "dot", width: 1 } },
                          ],
                        })}
                ...
                {/* ── TAB 5: SMART ENGINEERING INSIGHTS ── */}
                {activeTab === 5 && (
                  <div className="p-4 h-full overflow-y-auto custom-scrollbar">
                    <SmartInsights result={r} />
                  </div>
                )}

                {/* ── TAB 6: PARAMETRIC SENSITIVITY ANALYSIS ── */}
                {activeTab === 6 && (
          ...
          {hasResults && r && (
            <div className="h-64 shrink-0 bg-[#0d0d0f] border border-[#1e1e1e] rounded-xl overflow-hidden relative">
               ...
               {/* Live Telemetry: Quarter-car has ONE unsprung mass (z_u).
                   All 4 corners receive the same z_u since the model is 2-DOF.
                   This is a symmetric approximation for visualization only. */}
