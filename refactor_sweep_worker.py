...
# 1. Update sweep.py
sweep_path = os.path.join(project_dir, r"src\suspensionlab\backend\api\routes\simulate\sweep.py")
with open(sweep_path, "r", encoding="utf-8") as f:
    content = f.read()

# Replace the endpoint logic entirely
new_sweep = """from fastapi import APIRouter, HTTPException, Depends, Request
import json
import asyncio
...
new_worker = """                if job_type == "OPTIMIZE_QUARTER":
                    p = QuarterCarParams(**payload["params"])
                    r = RoadProfile(**payload["profile"])
                    objective = payload["objective_type"]
                    max_travel = payload["max_travel"]
